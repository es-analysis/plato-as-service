...
function makeResultsDir(options) {
    ...
    var resultsDir = join(REPORTS_DIR, options.user, options.repo, options.branch);
 ...
 * @returns {Promise}
 */
function isFresh(options, ttl) {
    var promise = vow.promise();
    var file = join(REPORTS_DIR, options.user, options.repo, options.branch, 'report.history.json');
...
exports.createReportFor = createReportFor;

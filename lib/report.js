...
var REPORTS_DIR = join(__dirname, '..', 'reports');
...
ReportGenerator.prototype = {
    ...
    create: function (options) {
        var self = this,
            key = [options.user, options.repo, options.branch].join('/'),
            pendingRequest = self.pendingRequests[key];

        if (pendingRequest) {
            return pendingRequest;
        }

        pendingRequest = self._create(options);

        pendingRequest.always(function () {
            delete self.pendingRequests[key];
        });

        return self.pendingRequests[key] = pendingRequest;
    },
    /**
     * @param {Object} options
     * @param {String} options.user
    ...
    _create: function (options) {
        ...
        return createTmpDir()
            .then(function (tmpDir) {
                return all([
                    extractTo(self.buildZipUrl(options), tmpDir),
                    self.makeResultsDir(options)
                ]);
            })
     ...
     */
    makeResultsDir: function (options) {
        var promise = vow.promise();

        var resultsDir = join(this.options.reports, options.user, options.repo, options.branch);
     ...
     * @returns {Promise}
     */
    isFresh: function (options) {
        var ttl = this.options.ttl;
        var promise = vow.promise();
        var file = join(this.options.reports, options.user, options.repo, options.branch, 'report.history.json');
    ...
    buildZipUrl: function (options) {
        return this.router.build(options);
    }
};
